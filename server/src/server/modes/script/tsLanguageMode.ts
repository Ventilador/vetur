...
import { LanguageRange } from '../../embeddedSupport/embeddedSupport';
...
export async function getJavascriptMode(): LanguageMode {
  ...
  return {
    ...
    updateFileInfo(doc: TextDocument): Promise<void> {
      return languageService.updateFile(doc);
    },

    async doValidation(doc: TextDocument): Diagnostic[] {
      if (!doc.uri.endsWith('vue')) {
        return [];
      }

      const fileFsPath = getFileFsPath(doc.uri);
      const rawScriptDiagnostics = await Promise.all([
        languageService.getSyntacticDiagnostics(fileFsPath),
        languageService.getSemanticDiagnostics(fileFsPath)
      ]);
