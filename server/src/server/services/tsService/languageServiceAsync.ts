...
export class ClientLanguageService extends AsyncLanguageService {
  ...
  protected initWrapper(metadata: IMetadataItem): Function {
    return argWrapper(metadata.transforms, function (args?: any[]) {
      if (bus) {
...
function createRetryingConnection() {
  const socket = createConnection({ port: VETUR_PORT, host: 'localhost' });
  return socket.on('error', function retry() {
    socket.end();
    if (bus) {
      bus.dispose();
      bus = undefined;
